  ...
  return (
    <div className="space-y-12 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col gap-4">
        ...
        <h1 className="text-[8rem] font-black tracking-tighter leading-[0.8] uppercase">
          DASH<br />BOARD
        </h1>
      </div>

      {/* Stats Grid */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-0 border-4 border-swiss-fg">
        {stats.map((stat, i) => (
          <div 
            key={i} 
            className={`p-8 border-swiss-fg ${i !== stats.length - 1 ? "lg:border-r-4" : ""} ${i < 2 ? "border-b-4 lg:border-b-0" : ""} group hover:bg-swiss-red transition-colors duration-150`}
          >
            <p className="text-[10px] font-black tracking-widest uppercase mb-8 group-hover:text-swiss-bg">{stat.label}</p>
            <div className="flex items-end justify-between">
              <h2 className="text-6xl font-black tracking-tighter group-hover:text-swiss-bg">{stat.value}</h2>
              <div className="flex flex-col items-end">
                <span className="text-xs font-bold text-swiss-red group-hover:text-swiss-bg flex items-center gap-1">
                  {stat.change} <ArrowUpRight className="w-3 h-3" />
                </span>
                <stat.icon className="w-6 h-6 mt-2 text-swiss-fg group-hover:text-swiss-bg" />
              </div>
            </div>
          </div>
        ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
        ...
        <div className="lg:col-span-8 border-4 border-swiss-fg p-12 bg-swiss-muted swiss-grid-pattern relative overflow-hidden group">
          <div className="flex justify-between items-start mb-12">
            <div>
              <p className="text-[10px] font-black tracking-widest uppercase mb-2">05. ISSUE TRENDS</p>
              <h3 className="text-4xl font-black tracking-tighter uppercase">REPORTING VELOCITY</h3>
            </div>
            <div className="flex gap-2">
              {['7D', '30D', '90D'].map((t) => (
                <button key={t} className="px-4 py-2 border-2 border-swiss-fg text-[10px] font-black tracking-widest hover:bg-swiss-fg hover:text-swiss-bg transition-colors">
                  {t}
                </button>
              ))}
            </div>
          ...
          <div className="h-64 flex items-end gap-2">
            {[40, 70, 45, 90, 65, 80, 50, 85, 40, 75, 95, 60].map((h, i) => (
              <div 
                ...
                style={{ height: `${h}%` }}
              >
                <div className="absolute -top-8 left-1/2 -translate-x-1/2 bg-swiss-fg text-swiss-bg text-[8px] px-1 py-0.5 opacity-0 group-hover/bar:opacity-100 transition-opacity">
                  {h}
                </div>
              </div>
            ))}
        ...
        <div className="lg:col-span-4 flex flex-col gap-4">
          <button className="flex-1 border-4 border-swiss-fg p-8 flex flex-col justify-between hover:bg-swiss-red group transition-colors text-left relative overflow-hidden">
            ...
            <h3 className="text-4xl font-black tracking-tighter uppercase leading-[0.9] group-hover:text-swiss-bg">
              NEW ISSUE<br />REPORT
            </h3>
            <ArrowUpRight className="w-8 h-8 self-end group-hover:text-swiss-bg" />
          </button>
          <button className="flex-1 border-4 border-swiss-fg p-8 flex flex-col justify-between hover:bg-swiss-fg group transition-colors text-left">
      ...
      <div className="border-4 border-swiss-fg overflow-hidden bg-swiss-bg">
        <div className="p-8 border-b-4 border-swiss-fg flex justify-between items-center bg-swiss-muted swiss-dots">
